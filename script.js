...
(() => {
  ...
  const validPhone = /^[1-9]\d{7,14}$/.test(contact.whatsapp) || /^https:\/\/wa\.me\//.test(contact.whatsapp);
  ...
  function setOrderContext(service = '') {
    ...
    if (validPhone) {
      const wa = /^https:\/\/wa\.me\//.test(contact.whatsapp) ? contact.whatsapp : 'https://wa.me/' + contact.whatsapp;
      externalLink(contactLink, wa + (wa.includes('?') ? '&' : '?') + 'text=' + encodeURIComponent(message));
    }
  ...
  if (validPhone) {
    ...
    $('#contact-number').textContent = /^https:\/\/wa\.me\//.test(contact.whatsapp) ? contact.whatsapp : '+' + contact.whatsapp;
    $('#contact-number').hidden = false;
    $('#contact-unavailable').hidden = true;
    setOrderContext();
  }
  const setQr = (container, source, alt) => {
    if (!source) return;
    ...
    image.width = 120;
    image.height = 120;
    container.querySelector('.qr-placeholder')?.remove();
    container.append(image);
  };
  const activateSocial = (channel, url, qr, label) => {
    if (!url) return;
    const link = $('#' + channel + '-link');
    externalLink(link, url);
    link.removeAttribute('aria-disabled');
    const card = link.closest('.social-card');
    card.classList.add('is-active');
    $('#' + channel + '-status').textContent = label;
    setQr($('#' + channel + '-qr'), qr, 'QR code menuju ' + label);
  };
  activateSocial('instagram', content.socials?.instagram, content.socials?.instagramQr, 'Instagram resmi GDT WORKS');
  activateSocial('shopee', content.socials?.shopee, content.socials?.shopeeQr, 'Toko Shopee resmi GDT WORKS');
  if (validPhone) {
    contactLink.removeAttribute('aria-disabled');
    contactLink.closest('.social-card').classList.add('is-active');
    setQr($('#whatsapp-qr'), contact.whatsappQr, 'QR code menuju WhatsApp GDT WORKS');
  }
  const socialCards = $$('.social-card');
  socialCards.forEach((card, index) => card.style.setProperty('--reveal-delay', (index * 120) + 'ms'));
  if (reducedMotion.matches || !('IntersectionObserver' in window)) socialCards.forEach((card) => card.classList.add('is-visible'));
  else {
    const socialObserver = new IntersectionObserver((entries, observer) => entries.forEach((entry) => {
  ...
  projects.forEach((project, index) => {
    ...
    visual.append(img, make('span', 'concept-label', project.concept ? 'GDT works' : 'PROYEK PILIHAN'), number, arrow);
  ...
  content.progress.forEach((entry) => {
    ...
    const details = make('div', 'order-details');
    card.append(details);
    if (entry.photos?.length) {
      const gallery = make('div', 'order-gallery');
      gallery.setAttribute('role', 'region');
      gallery.setAttribute('aria-label', 'Foto pesanan ' + entry.title);
      gallery.tabIndex = 0;
      const slides = entry.photos.map((photo, index) => {
        const img = make('img');
        img.src = safeImage(photo.image);
        img.alt = photo.alt;
        img.width = 1536;
        img.height = 864;
        img.loading = 'lazy';
      ...
      const controls = make('div', 'slide-controls');
      ...
      const img = make('img', 'order-photo');
    ...
    details.append(make('span', 'mono', entry.projectCode), make('h3', '', entry.title), make('p', '', entry.description));
    const statusLabels = { production: 'Proses pengerjaan', shipping: 'Proses pengiriman', delivered: 'Pesanan diterima' };
    const progressHeader = make('div', 'order-progress-heading');
    progressHeader.append(make('span', '', statusLabels[entry.status] || statusLabels.production), make('strong', '', (entry.percentage || 0) + '%'));
    const meter = make('progress', 'order-meter');
    meter.max = 100;
    meter.value = entry.percentage || 0;
    meter.setAttribute('aria-label', 'Pengerjaan ' + entry.title);
    details.append(progressHeader, meter);
    const updated = make('time', 'progress-date', dateLabel(entry.date));
    updated.dateTime = entry.date;
    details.append(make('div', 'progress-stage', entry.stage), updated);
  ...
  // Reveal only once; content stays visible without JavaScript or with reduced motion.
  const revealTargets = $$('.hero-copy, .section-heading, .about-copy, .services-heading, .service-list, .progress-card, .progress-empty, .location-grid, .contact-heading, .contact-intro');
  if ('IntersectionObserver' in window && !reducedMotion.matches) {
    ...
    revealTargets.forEach((element) => { element.classList.add('fade-once'); revealObserver.observe(element); });
  }
